...
function createWriteWrap(handle) {
  ...
  req.handle = handle;
  req.oncomplete = onWriteComplete; // XXX this isn't called?
  req.async = false;
  req.bytes = 0;
  req.buffer = null;
...
function writeGeneric(self, req, data, encoding, cb) {
  // XXX this has to be wrong... but when I do this it fixes dozens of failures
  // :-( But it breaks other things.
  // Something is going wrong with .end(data) so that this callback doesn't
  // occur, and while data is written, the actual .end() after data does not do
  // a close_notify&TCP close
  // req.callback = cb;
