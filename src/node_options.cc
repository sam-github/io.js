...
EnvironmentOptionsParser::EnvironmentOptionsParser() {
  ...
  // - 11.x: XXX would like it to be opt-out, but might need to be opt-out if
  //   if its too disruptive
  // - 12.x: TLS1.3 is opt-out with --tls-max-v1.2
  // Support both options wherever 1.3 is possible, so they are uniformly
  // available.
